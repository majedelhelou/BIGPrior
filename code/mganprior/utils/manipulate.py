...
def AWGN_images(image, args):
    if args.single_level:
        sigma = args.sigma_min
    else:
        sigma = rand.randint(args.sigma_min, args.sigma_max)
    #the factor of 2 is for tanh [-1,1] range
    noise = torch.FloatTensor(image.shape).normal_(mean=0, std=2*sigma/255.)
    return image+noise.cuda(), sigma
